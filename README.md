...
Covers **2GIS**, **Yandex Maps**, **Yandex Realty**, **Drom.ru**, **CIAN**, **Krisha.kz** and **Kolesa.kz**. Pull listings and business data into clean CSV + JSON. You decide what to collect.
...
| [`drom/`](drom/) | [Drom.ru](https://auto.drom.ru) | RU used cars | Direct HTTP / list JSON | optional | Not required |
...
- **Drom / Kolesa** — used cars (RU / KZ). Drom phones need a session (contacts API usually blocked anonymously).
...
| Drom.ru | no user API key; phones usually blocked without a Drom session |
...
- **Drom** — `auto.drom.ru/{city}/all/` + `/pageN/`; `data-drom-module="bulls-list-auto"` JSON. Contacts API without session returns `type=4`.
...
See [CHANGELOG.md](CHANGELOG.md) for release notes (`v0.4.0`).
